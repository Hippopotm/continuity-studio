...
export default function Home() {
  ...
  const [connection, setConnection] = useState<Connection>({ provider: "openai", provider_api_key: "", b2_key_id: "", b2_app_key: "", b2_bucket: "ContinuityProject", b2_endpoint: "https://s3.us-west-004.backblazeb2.com" });
  ...
  return (
    <main className="studio-shell">
      ...
      {showConnection && <div className="modal-backdrop" onClick={() => setShowConnection(false)}><section className="connection-modal-new" onClick={(event) => event.stopPropagation()}><header><div><span className="eyebrow">BRING YOUR OWN KEYS</span><h2>Connect your creative stack</h2><p>Credentials are used only for connection tests and generation requests. They are never included in provenance records.</p></div><button onClick={() => setShowConnection(false)}>×</button></header><div className="connection-form"><label>Video provider<select value={connection.provider} onChange={(e) => setConnection({ ...connection, provider: e.target.value })}><option value="openai">OpenAI · Sora 2</option><option value="gmicloud">GMI Cloud · Kling</option></select></label><label>Provider API key<input type="password" autoComplete="off" value={connection.provider_api_key} onChange={(e) => setConnection({ ...connection, provider_api_key: e.target.value })} placeholder="Enter provider key" /></label><div className="form-divider"><span>BACKBLAZE B2 STORAGE</span></div><label>Key ID<input type="password" autoComplete="off" value={connection.b2_key_id} onChange={(e) => setConnection({ ...connection, b2_key_id: e.target.value })} placeholder="Application key ID" /></label><label>Application key<input type="password" autoComplete="off" value={connection.b2_app_key} onChange={(e) => setConnection({ ...connection, b2_app_key: e.target.value })} placeholder="Application key" /></label><label>Bucket<input value={connection.b2_bucket} onChange={(e) => setConnection({ ...connection, b2_bucket: e.target.value })} /></label><label>S3 endpoint<input value={connection.b2_endpoint} onChange={(e) => setConnection({ ...connection, b2_endpoint: e.target.value })} /></label></div><footer><span><i /> Session-only credentials</span><button className="primary-button" onClick={connect} disabled={connecting}>{connecting ? "Verifying stack…" : "Verify & connect"}<b>↗</b></button></footer></section></div>}
