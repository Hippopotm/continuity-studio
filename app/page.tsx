...
function playableAssetUrl(url?: string) { const stable = stableAssetUrl(url); return stable ? `/api/assets/play?url=${encodeURIComponent(stable)}` : undefined; }
...
export default function Home() {
  ...
  const generatedVideo = videoAsset(run)?.url || playableAssetUrl(selectedGeneration?.storageVideoUrl || selectedGeneration?.videoUrl);
  const generatedPoster = playableAssetUrl(selectedGeneration?.storageFinalFrameUrl || selectedGeneration?.finalFrameUrl);
  const finalPlayableUrl = playableAssetUrl(project.finalVideoStorageUrl || project.finalVideoUrl);
