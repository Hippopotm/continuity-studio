...
type CharacterReferences = { front?: string; threeQuarter?: string; profile?: string; body?: string; clothes?: string };
...
function refLabel(key: keyof CharacterReferences) { return ({ front: "Front face", threeQuarter: "3/4 face", profile: "Profile", body: "Full body", clothes: "Clothes" } as const)[key]; }
...
function migrateProject(raw: Partial<Project> & { brief?: string; shots?: Array<Partial<Shot> & { copy?: string }> }): Project {
  return {
    ...
    characterReferences: raw.characterReferences || {},
...
export default function Home() {
  ...
  const referencePrompts = {
    ...
    clothes: `wardrobe detail sheet, exact clothes, fabric texture, colors, accessories, no face change`,
  };
  const continuityJson = {
    ...
    character: {
      ...
      references: Object.fromEntries(Object.entries(project.characterReferences).map(([key, value]) => [key, { uploaded: Boolean(value), role: refLabel(key as keyof CharacterReferences) }])),
      reference_generation_prompts: referencePrompts,
    },
    shot: { id: current.id, title: shotTitle(current), duration_seconds: current.duration, brief: current.brief },
  ...
  async function generate() {
    ...
    try {
      const response = await fetch("/api/runs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ project_id: project.id, shot_id: `shot-${current.id}`, provider: "openai", model: "sora-2", specification: continuityJson, reference_urls: [], previous_clean_frame_url: previousSelected?.finalFrameUrl || null, budget_usd: Math.max(Number(cost), 0.1), connection }) });
    ...
    <section className="studio">
      ...
      <div className="creation-grid">
        <aside className="story-panel glass-panel"><div className="section-kicker"><span>01</span> STORY DIRECTION</div><label className="input-label" htmlFor="brief">Scene brief for this shot</label><textarea id="brief" value={current.brief} onChange={(e) => updateShot({ brief: e.target.value })} placeholder="Write only what should happen in this exact shot..." /><button className="compile-button" onClick={() => setShowSpec(true)}>✦ Compile continuity lock <span>JSON</span></button><div className="divider" />
          ...
          <div className="reference-grid expanded">{(["front", "threeQuarter", "profile", "body", "clothes"] as Array<keyof CharacterReferences>).map((key) => <label key={key} className="reference-upload"><input type="file" accept="image/*" onChange={(e) => uploadRef(key, e)} />{project.characterReferences[key] ? <img src={project.characterReferences[key]} alt={refLabel(key)} /> : <span>＋<small>{refLabel(key)}</small></span>}</label>)}</div>
        </aside>
